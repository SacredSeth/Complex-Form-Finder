...
def convert_roots(start_root):
    ...
    if start_root[-1] == 'real':  # real roots are in form (value, form)
        ...
        if abs(angle) < math.pi / 2:  # acute angles
            ...
            rea_form = magnitude, 'real' if angle == 0 else -magnitude, 'real'  # redefine real value
            imaginary = 0
            real = magnitude
...
def print_roots(list_of_roots):
    ...
    if list_of_roots != list(list_of_roots):
        list_of_roots = list(list_of_roots)
    ...
    for item in list_of_roots:
        ...
        match item[-1]:
            ...
            case 'polar':
                ...
                if item[1] == math.pi:
                    final = f"{item[0]} cis \u03C0", 'polar'
                else:
                    final = f"{item[0]} cis {angle}", 'polar'
            ...
            case 'exponential':
                ...
                if item[1] == math.pi:
                    final = f"{item[0]} e^(\u03C0i)", "exponential"
                else:
                    final = f"{item[0]} e^({angle} i)", "exponential"
...
if equa_or_root == "equation":  # convert from equation
    ...
    for _ in range(how_many):
        ...
        print(f"\nYour equation:", simple_equation(coefficient_list))
        ...
        expo_list = []
        for root in roots:
            real_form, rect_form, polar_form, expo_form = print_roots(convert_roots(root))
    ...
    for item in root_list:
        converted_list.append(convert_roots(item))

    list_of_print = []
    for item in converted_list:
        list_of_print.append(print_roots(item))

    for to_print in list_of_print:
        print(to_print)
